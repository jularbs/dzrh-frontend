...
import { AUDIO_STREAMING_URL } from "../../../../helpers/constants";
...
const AudioStreamingPage = () => {
  ...
  const init = () => {
    read(AUDIO_STREAMING_URL).then((data) => {
      if (!data) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    //show loading
    update({ key: AUDIO_STREAMING_URL, value: url }, token).then((data) => {
      if (data.error) {
        setValues({ ...values, error: data.error });
      }
  ...
  const showSuccess = () =>
    success && (
      <div className="alert alert-success">
        "Audio Livestream URL updated successfully."
      </div>
    );

  const showAudioStreamForm = () => {
    return (
      <Card small className="mb-3">
        <CardHeader className="border-bottom">
          <h6 className="m-0">Manage Audio Livestreaming</h6>
        </CardHeader>
        <CardBody>
          {showError()}
          {showSuccess()}
